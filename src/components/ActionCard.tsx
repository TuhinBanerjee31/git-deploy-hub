...
export function ActionCard({ icon: Icon, title, description, active = false, blinking = false }: ActionCardProps) {
  return (
    <div className={cn(
      ...
      blinking && "animate-[pulse_2s_ease-in-out_infinite] border-green-400 bg-green-500/20"
    )}>
      <div className="flex flex-col items-center text-center space-y-3">
        <Icon className={cn(
          "w-8 h-8 text-green-400",
          blinking && "animate-[bounce_3s_ease-in-out_infinite]"
        )} />
        <h3 className={cn(
          "text-green-400 font-semibold text-sm tracking-wider uppercase",
          blinking && "animate-[pulse_2.5s_ease-in-out_infinite]"
        )}>
          {title}
        </h3>
